...
void log(std::string text, LOG_LEVEL level)
{
    ...
    if (level == LOG_LEVEL::NORMAL)
        std::println("[{:%OH:%OM:%OS}] {}", std::chrono::system_clock::now(), text);
    else if (level == LOG_LEVEL::WARNING)
        std::println("[{:%OH:%OM:%OS}] \x1b[1;33m{}\033[0m",std::chrono::system_clock::now(), text);
    else if (level == LOG_LEVEL::ERROR)
        std::println("[{:%OH:%OM:%OS}] \x1b[1;31m{}\033[0m", std::chrono::system_clock::now(),text);
}
